...
class TestMapping < LinkedData::TestOntologyCommon
  ...
  def test_mappings_ontology
    ...
    assert_includes [10, 5], by_ont_counts["MAPPING_TEST2"] # if owlapi import SKOS concepts
    assert_includes [8,3], by_ont_counts["MAPPING_TEST4"] # if owlapi import SKOS concepts
    assert_includes [18,8,13], total
    assert_includes [18,8, 13], mappings.length
    assert_includes [10,0, 5], same_uri
    assert_equal(cui, 3)
    assert_equal(loom,5)
    mappings.each do |map|
  ...
  def test_mappings_two_ontologies
    ...
    assert_equal(5, same_uri) unless same_uri.zero? # if owlapi import SKOS concepts
    assert_equal(1, cui)
    assert_equal(2, loom)
  end
